function slice = ComputeOrthogonalSlice(Image, orientation, position, res_1, res_2, method)
...
if strcmp(orientation,'X-Y') == 1
    ...
    % 2) Check position is within bounds
    ...
    % 3) convert position to a slice number to index original volume with
    % Assuming slice 1 is centred around 0 mm
    z_query = (position/vox_length) + 1;
     
    % 4) Interpolate according to string contained in method
